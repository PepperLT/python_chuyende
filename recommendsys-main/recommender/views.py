...
def recs_funksvd(request, user_id, num=6):
    """FunkSVD recommendations API endpoint"""
    try:
        # Khởi tạo và set path
        funksvd = FunkSVDRecs()
        funksvd.set_save_path('./models/funkSVD/model/')
        
        # Lấy recommendations
        sorted_items = funksvd.recommend_items(user_id, num)
        
        print(f"✅ FunkSVD API: Found {len(sorted_items)} recs for user {user_id}")
        ...
        return JsonResponse({
            'user_id': user_id,
            'data': [],
            'error': 'Model not trained yet. Please run: python -m builder.matrix_factorization_calculator'
        }, safe=False)
        
    except Exception as e:
        print(f"❌ FunkSVD API Error: {e}")
        import traceback
        traceback.print_exc()
        return JsonResponse({
            'user_id': user_id,
